  ...
  <div class="app-container">
    ...
    <el-card class="mt20">
      ...
      <el-row :gutter="10" class="mb8" type="flex" justify="space-between">
        <el-col :span="6"><span style="font-size: large">采购退货单明细</span></el-col>
        <el-col :span="1.5">
          <el-button
            type="primary"
            plain
            icon="Plus"
            @click="handleAdd"
            v-hasPermi="['purchase:refundDetail:add']"
          >新增</el-button>
          <el-button
            type="warning"
            plain
            icon="Download"
            @click="handleExport"
            v-hasPermi="['purchase:refundDetail:export']"
          >导出</el-button>
        </el-col>
      </el-row>

      <el-table v-loading="loading" :data="refundDetailList" border class="mt20">
        ...
        <el-table-column label="操作" align="right" class-name="small-padding fixed-width">
            <template #default="scope">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['purchase:refundDetail:edit']">修改</el-button>
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['purchase:refundDetail:remove']">删除</el-button>
            </template>
        </el-table-column>
      </el-table>
...
<script setup name="RefundDetail">
import { listRefundDetail, getRefundDetail, delRefundDetail, addRefundDetail, updateRefundDetail } from "@/api/purchase/refundDetail";
...
function getList() {
  loading.value = true;
  listRefundDetail(queryParams.value).then(response => {
    refundDetailList.value = response.rows;
...
function reset() {
  ...
  proxy.resetForm("refundDetailRef");
}
...
function handleUpdate(row) {
  ...
  getRefundDetail(_id).then(response => {
    form.value = response.data;
    open.value = true;
    title.value = "修改采购退货单明细";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["refundDetailRef"].validate(valid => {
    if (valid) {
      buttonLoading.value = true;
      if (form.value.id != null) {
        updateRefundDetail(form.value).then(response => {
        ...
        addRefundDetail(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除采购退货单明细编号为"' + _ids + '"的数据项？').then(function() {
    loading.value = true;
    return delRefundDetail(_ids);
  }).then(() => {
    loading.value = true;
    getList();
...
function handleExport() {
  proxy.download('purchase/refundDetail/export', {
    ...queryParams.value
  }, `refundDetail_${new Date().getTime()}.xlsx`)
}
