  ...
  <div class="app-container">
    ...
    <el-card class="mt20">
      ...
      <el-row :gutter="10" class="mb8" type="flex" justify="space-between">
        <el-col :span="6"><span style="font-size: large">采购订单明细</span></el-col>
        <el-col :span="1.5">
          <el-button
            type="primary"
            plain
            icon="Plus"
            @click="handleAdd"
            v-hasPermi="['purchase:orderDetail:add']"
          >新增</el-button>
          <el-button
            type="warning"
            plain
            icon="Download"
            @click="handleExport"
            v-hasPermi="['purchase:orderDetail:export']"
          >导出</el-button>
        </el-col>
      </el-row>

      <el-table v-loading="loading" :data="orderDetailList" border class="mt20">
        ...
        <el-table-column label="操作" align="right" class-name="small-padding fixed-width">
            <template #default="scope">
                <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['purchase:orderDetail:edit']">修改</el-button>
                <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['purchase:orderDetail:remove']">删除</el-button>
            </template>
        </el-table-column>
      </el-table>
    ...
    <el-drawer :title="title" v-model="open" size="50%" append-to-body>
      <el-form ref="orderDetailRef" :model="form" :rules="rules" label-width="80px">
        <el-form-goods label="父id" prop="pid">
...
import { listOrderDetail, getOrderDetail, delOrderDetail, addOrderDetail, updateOrderDetail } from "@/api/purchase/orderDetail";
...
function getList() {
  loading.value = true;
  listOrderDetail(queryParams.value).then(response => {
...
function reset() {
  ...
  proxy.resetForm("orderDetailRef");
}
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["orderDetailRef"].validate(valid => {
    if (valid) {
      buttonLoading.value = true;
      if (form.value.id != null) {
        updateOrderDetail(form.value).then(response => {
        ...
        addOrderDetail(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除采购订单明细编号为"' + _ids + '"的数据项？').then(function() {
    loading.value = true;
    return delOrderDetail(_ids);
  }).then(() => {
    loading.value = true;
    getList();
...
function handleExport() {
  proxy.download('purchase/orderDetail/export', {
    ...queryParams.value
